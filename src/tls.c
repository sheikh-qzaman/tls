...
/**
 * Turn the premaster secret into an actual master secret (the server side will do this
 * concurrently) as specified in section 8.1:
 * master_secret = PRF( pre_master_secret, "master secret", ClientHello.random + ServerHello.random );
 * ( premaster_secret, parameters );
 * Note that, with DH, the master secret len is determined by the generator (p) value.
 */
                                  ...
                                  TLSParameters         *parameters )
{
    ...
    PRF(premaster_secret, premaster_secret_len, label, strlen( label ),
...
static void calculate_keys( TLSParameters *parameters )
{
    ...
    int                       key_block_length = suite->hash_size * 2 +
                            suite->key_size * 2 + suite->IV_size * 2;
    ...
    PRF(parameters->master_secret, MASTER_SECRET_LENGTH, label, strlen( label ),
        seed, RANDOM_LENGTH * 2, key_block, key_block_length );
...
/*
 * TLS mandates not only that every handshake message be prepended with a header indicating its type and length,
 * but that every message, including the already-prepended handshake messages, be prepended with yet another header
 * indicating its type and length. There are four types of TLS messages defined: handshake messages, alerts, data
 * and "change cipher spec". Also, the protocol version is included on every packet.
 */
static int
send_message(int                    connection,
             int                    content_type,
...
{ 
    ...
    /* This checks whether we should encrypt the outgoing message, after key exchange successfull.
     */
    if ( active_suite->new_digest ) { 
        ...
        hmac( parameters->MAC_secret, active_suite->hash_size, mac_buffer, 13 + content_len, &digest );
    ...
    if ( padding_length > 0 ) {
        ...
        for (padding = send_buffer + send_buffer_size - 1;
                padding > ( send_buffer + ( send_buffer_size - padding_length - 1 ) ); padding--) {
    ...
    if ( active_suite->bulk_encrypt ) {
        ...
        /* The first 5 bytes (the header) aren't encrypted; this is necessary so that the receiver
         * knows how many bytes to process in the current packet.
         */
        memcpy( encrypted_buffer, send_buffer, 5 );
        active_suite->bulk_encrypt( send_buffer + 5, send_buffer_size - 5, encrypted_buffer + 5,
...
/* The change cipher spec message is a marker message, just like server hello done was,
 * that doesn't include any data. It is a major milestone in the handshake process,
 * because the reception of a change cipher spec message tells the other side that the
 * key exchange has completed, and every subsequent message sent by this peer is encrypted
 * and authenticated using it. So, at this point in the key exchange, the client should send
 * a change cipher spec and make its pending send parameters active, but not touch the
 * pending receive parameters.
 */
...
{ 
    ...
    /* Per 6.1: The sequence number must be set to zero whenever a connection state is made the active state.
     * The first record which is transmitted under a particular connection state should use sequence number 0.
     */
    parameters->pending_send_parameters.seq_num = 0;

    memcpy( &parameters->active_send_parameters, &parameters->pending_send_parameters,
...
void compute_handshake_hash( TLSParameters *parameters, unsigned char *handshake_hash )
{
    ...
    */
    memcpy( &tmp_md5_handshake_digest, &parameters->md5_handshake_digest, sizeof( digest_ctx ) );
    memcpy( &tmp_sha1_handshake_digest, &parameters->sha1_handshake_digest, sizeof( digest_ctx ) );
    ...
    memcpy( tmp_md5_handshake_digest.hash, parameters->md5_handshake_digest.hash, MD5_BYTE_SIZE );
    memcpy( tmp_sha1_handshake_digest.hash, parameters->sha1_handshake_digest.hash, SHA1_BYTE_SIZE );
...
/**
 * 7.4.9:
 * verify_data = PRF(master_secret, "client finished", MD5(handshake_messages) + SHA-1(handshake_messages)) [0..11]
 * master_secret = PRF(pre_master_secret, "master secret", ClientHello.random + ServerHello.random);
...
static void
compute_verify_data(const char      *finished_label,
                    TLSParameters   *parameters,
                    char            *verify_data )
{
    ...
    PRF(parameters->master_secret, MASTER_SECRET_LENGTH, finished_label, strlen( finished_label ),
        handshake_hash, MD5_RESULT_SIZE * sizeof( int ) + SHA1_RESULT_SIZE * sizeof( int ),
        verify_data, VERIFY_DATA_LEN );
}

/*
* Each handshake message must be prepended with a header indicating its type and length.
*/
static int
send_handshake_message(int                   connection,
                       int                   msg_type,
...
static int
send_finished( int connection, TLSParameters *parameters )
{
...
parse_finished(unsigned char    *read_pos,
               int              pdu_length,
               TLSParameters    *parameters)
{
    unsigned char verify_data[ VERIFY_DATA_LEN ];
    ...
    // Recompute the verification data and compare with the received data.
 ...
 * The goal of the key exchange is to exchange a premaster secret,
 * turn it into a master secret, and use that to calculate the keys
 * that are used for the remainder of the connection.
 */ 
...
{   
    ...
    switch ( parameters->pending_send_parameters.suite ) {
        ...
        case TLS_RSA_WITH_AES_256_CBC_SHA:
            ...
            key_exchange_message_len = rsa_key_exchange(&parameters->server_public_key.rsa_public_key,
    ...
    if ( send_handshake_message(connection, client_key_exchange, key_exchange_message,
                                key_exchange_message_len, parameters)) {
    ...
    /* Now, turn the premaster secret into an actual master secret
     * the server side will do this concurrently.
     */
    compute_master_secret( premaster_secret, premaster_secret_len, parameters );

    // XXX - for security, should also "purge" the premaster secret from memory.
 ...
 */
static int
send_client_hello( int connection, TLSParameters *parameters )
{
    ClientHello       package;
     ...
     * This is 28 bytes, but client random is 32, the first four bytes of "client random"
     * are the GMT unix time computed above.
     */
    ...
    // Indicating that a new session is being requested.
    ...
    send_buffer_size = sizeof( ProtocolVersion ) + sizeof( Random ) + sizeof( unsigned char ) +
                       ( sizeof( unsigned char ) * package.session_id_length ) + sizeof( unsigned short ) +
                       ( sizeof( unsigned short ) * 1 ) + sizeof( unsigned char ) + sizeof( unsigned char );
...
static int send_alert_message( int connection, int alert_code, ProtectionParameters *parameters ) {
    char buffer[ 2 ];
...
static char *parse_server_hello( char *read_pos, int pdu_length, TLSParameters *parameters )
{
    ServerHello hello;
    ...
    // *DON'T* put this in host order, since it's not used as a time!  Just accept it as is
    ...
    /* TODO - abort if there's more data here than in the spec (per section 7.4.1.2, forward compatibility note)
     * TODO - abort if version < 3.1 with "protocol_version" alert error 28 random bytes, but the preceding four
     * bytes are the reported GMT unix time
     */
...
{ 
    ...
    *decrypted_message = ( unsigned char * ) malloc( encrypted_length );
    ...
    if ( active_suite->bulk_decrypt ) {
        active_suite->bulk_decrypt( encrypted_message, encrypted_length, *decrypted_message,
                                    parameters->IV, parameters->key );
    ...
    if ( active_suite->new_digest ) {
        ...
        /* Copy first three bytes of header; last two bytes reflected the message length,
         * with MAC attached.  Since the MAC was computed by the other side before it was
         * attached (obviously), that MAC was computed using the original length.
         */
        ...
        if (memcmp(digest.hash, (*decrypted_message) + decrypted_length, digest.hash_len * sizeof(int))) {
            return -1;
        }
 ...
 * to update the security parameters based on the type of message received. If the read
 * times out, or if an alert is received, return an error code; return 0 on success.
 ...
 */
static int receive_tls_msg( int connection, char *buffer, int bufsz, TLSParameters *parameters )
{
  ...
  int                   decrypted_length;
    ...
    /*
    * STEP 1 - read off the TLS Record layer. First, check to see if there's any data
    * left over from a previous read. If there is, pass that back up. This means that
    * if the caller isn't quick about reading available data, TLS alerts can be missed.
    */
    if ( parameters->unread_buffer != NULL ) {
        ...
        /* Because TCP doesn't guarantee that all bytes are available right away, it's necessary
         * to enter a receive loop, keep looping & appending until all bytes are accounted for
         * This loop, as presented here, is vulnerable to a denial of service attack. If the server
         * announces that 100 bytes are available but never sends them, the client hangs forever
         * waiting for these bytes. There's not much you can do about this, though. You can (and should)
         * set a socket-level timeout, but if it expires, there's not much point in continuing the connection.
         */
        accum_bytes = 0;
        ...
        /* If a cipherspec is active, all of "encrypted_message" will be encrypted. Must decrypt
         * it before continuing. This will change the message length in all cases, since decrypting
         * also involves verifying a MAC (unless the active cipher spec is NULL_WITH_NULL_NULL).
         */
        decrypted_message = NULL;
        decrypted_length = tls_decrypt( header, encrypted_message, message.length, &decrypted_message,
                                        &parameters->active_recv_parameters );
    ...
    if ( message.type == content_handshake ) {
        /* TLS format allows either side to concatenate multiple handshake messages within a single TLS message.
         * This capability was one of the big benefits of TLS over SSLv2. TLS header that includeds a message
         * length which is then followed by the seemingly superfluous handshake header that included essentially
         * the same length. This is why it was done this way; a single TLS header can identify multiple handshake
         * messages, each with its own independent length.
         */
        while ( ( read_pos - decrypted_message ) < decrypted_length ) {
            ...
            switch ( handshake.msg_type ) {
                ...
                default:
                    ...
                    * TODO However, out-of-order messages should result in a fatal alert per section 7.4
                    */
        ...
        while ( ( read_pos - decrypted_message ) < decrypted_length ) {
            ...
            /* If the server specifically marks an alert as a fatal, the handshake is aborted; otherwise, the
             * handshake process continues. Effectively this means that this implementation is ignoring warnings.
             */
        ...
        while ( ( read_pos - decrypted_message ) < decrypted_length ) {
            ...
            if ( change_cipher_spec_type != 1 ) {
                ...
                memcpy(&parameters->active_recv_parameters, &parameters->pending_recv_parameters,
                       sizeof(ProtectionParameters));
                init_protection_parameters( &parameters->pending_recv_parameters );
            }
        }
        ...
        if ( decrypted_length <= bufsz ) {
            ...
            // Need to hang on to a buffer of data here and pass it back for the next call
...
int tls_connect( int connection, TLSParameters *parameters )
{
    ...
    parameters->server_hello_done = 0;
    /* A client hello message and then received the server hello. Due to piggy-backing
     * of handshake messages, that call to receive probably picked up all three expected
     * messages and processed them (server hello, certificate, server done), culminating
     * in the setting of server_hello_done. This isn't guaranteed, though; the server could
     * legitimately split these up into three separate messages. To handle either case,
     * keep receiving TLS messages until server_hello_done is set.
     */
